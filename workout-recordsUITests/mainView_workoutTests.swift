...
class mainView_workoutTests: XCTestCase {
    ...
    private let energyDate = { return MainPageObject.formatDateHour(Date()) }()
    ...
    func testDeleteSecondItem() {
        ...
        XCTAssertEqual(mainPage().getWorkout(0).getEnergy(), "33")
        XCTAssertEqual(mainPage().getWorkout(1).getEnergy(), "11")
    }
    
    func testShowMoreWorkouts() {
        mainPage().createWorkout(activity: "Cycling (distance + energy)", (daysPast: 8, hour: 10, min: (14)), distance: 4.4, energy: 44)
        XCTAssertEqual(mainPage().workoutCount(), 2)
        mainPage().showMore()
        XCTAssertEqual(mainPage().workoutCount(), 3)
